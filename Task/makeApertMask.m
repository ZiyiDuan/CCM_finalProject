...
function [mask, maskGrey] = makeApertMask(screenP, exptP)
        ...
        x = x-mean(x(:));
        y = y-mean(y(:));
        [th,rad] = cart2pol(x,y);   % change the cartesian coor into polar coor
        mask(rad < exptP.outApert/2 * screenP.pixels_per_deg_height) = 1;   % change the aperture into 1(white)
